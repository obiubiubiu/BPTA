...
struct CURRENT_WORKER_STATE // 当前工人的状态6个
{
    ...
    vector<double> matched_MaxDistanceTask_orig; // 任务剩余允许的最大绕路距离：(end-start)*speed（总起始）-【current_task_detour_distance[i]自绕-前taskCost（绕）（到目标点的消耗）】-（now-start）*speed(时间)
};
struct CURRENT_TASK_STATE // 当前工人状态3个
{
    ...
    double MaxDistanceTask_orig;                 // 任务剩余允许的最大绕路距离
};
...
public:
    ...
    void Compute_global_PTPW_Group(vector<vector<pair<int, double>>> &PT, vector<vector<pair<int, double>>> &PW); // 计算全局的工人和任务的偏好
    ...
    // 时间相关
    void Initialize_group(vector<double> &AD, vector<double> &Sumdis, int current_workID, double startTime, vector<CURRENT_WORKERS_GROUP> &current_workerGroup);
    ...
    void UpdateTaskDeadline(int workerid, int taskid, vector<double> current_detour_distance[], vector<int> CT_Worker[], vector<int> poi[], double MaxDistanceTask[], double current_task_NeedTime);
    ...
    void UpdateTaskDeadline_TSDA(bool replace, int replaceid, int workerid, int taskid, vector<double> current_detour_distance[], vector<vector<int>> &CT_Worker, vector<int> poi[], double MaxDistanceTask[], double current_task_NeedTime);
